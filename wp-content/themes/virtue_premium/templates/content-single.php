        ...
        <?php } else if ($headcontent == 'image') {           
                ...
                $image = aq_resize( $img_url, $slidewidth, $slideheight, true ); //resize & crop the image
                    if(empty($image)) { $image = $img_url; }
                      if($image) : ?>
                        <div class="imghoverclass post-single-img" itemprop="image"><a href="<?php echo esc_url($img_url); ?>" rel="lightbox" class=""><img src="<?php echo esc_url($image); ?>" alt="<?php the_title(); ?>" /></a></div>
                      <?php endif; ?>
        <?php } ?>
